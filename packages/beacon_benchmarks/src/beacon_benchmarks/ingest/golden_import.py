...
def _gold_answer(expected: Any) -> dict[str, Any] | None:
    ...
    The shape is serde's internally-tagged form — ``{"kind": "number", "value":
    …}`` — because that is what ``GoldenExpectedResult`` actually serializes to.
    The first version of this importer parsed the Rust *type names*
    (``{"Number": {…}}``), a format nothing produces: a real export imported
    zero questions, and only a fixture generated by verity's own serde caught it.
    """
    if not isinstance(expected, dict):
        return None
    kind = expected.get("kind")
    if kind in ("text", "number"):
        return {"answer": expected.get("value")}
    if kind == "table":
        return {"columns": expected.get("columns", []), "rows": expected.get("rows", [])}
    return None
...
def _tolerance(raw: Any) -> dict[str, Any] | None:
    ...
    if not isinstance(raw, dict):
        return None
    out: dict[str, Any] = {}
    # Optional fields serialize as null, which is "no curated opinion", not a
    # value -- so only genuine values are carried across.
    if isinstance(raw.get("numeric_abs"), int | float):
        out["numeric_abs"] = float(raw["numeric_abs"])
    if isinstance(raw.get("numeric_rel"), int | float):
        out["numeric_rel"] = float(raw["numeric_rel"])
